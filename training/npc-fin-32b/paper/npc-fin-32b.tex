...
We use \texttt{Qwen/Qwen2.5-32B-Instruct} as the base. The base model is loaded in 4-bit NF4 quantization (bitsandbytes), and a LoRA adapter is added in bf16 over all seven projection modules in each transformer block. Table~\ref{tab:hp} summarizes the adapter and training-loop hyperparameters as declared in the training YAML.
...
The DeepSpeed config sets all three batch-related fields (\texttt{train\_batch\_size}, \texttt{gradient\_accumulation\_steps}, and the per-GPU micro-batch) to \texttt{"auto"}, deferring to the runtime. When the YAML's micro-batch and grad-accum values are propagated through DeepSpeed's auto-resolution at 12-GPU world size, the realized global effective batch becomes:
...
Table~\ref{tab:run} summarizes the realized run.
