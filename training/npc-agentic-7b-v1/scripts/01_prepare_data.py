...
def build_identity_examples(n_target: int) -> List[Dict[str, Any]]:
    """
    Generate `n_target` identity examples by expanding each seed prompt
    with simple templated variations (casing, punctuation, prefixes, tails).
    ...
    tails = config.IDENTITY_TAILS

    # Expand: each seed × (prefixes × suffixes × casing) — plenty of room
    # to hit 75 variants per seed without collisions.
    variants_per_seed = max(1, n_target // len(prompts))
    ...
    for seed in prompts:
        count = 0
        attempts = 0
        while count < variants_per_seed and attempts < variants_per_seed * 10:
            attempts += 1
            pfx = rng.choice(prefixes)
            sfx = rng.choice(suffixes)
            # Drop the seed's default trailing punctuation when applying our own suffix
            base = seed.rstrip("?.!")
            ...
            if prompt in seen:
                continue
            seen.add(prompt)
            reply = config.IDENTITY_RESPONSE_TEMPLATE + rng.choice(tails)
            out.append({
                "messages": coerce_messages(
                    system=config.IDENTITY_SYSTEM,
                    user=prompt,
                    assistant=reply,
                ),
                "source": "identity",
...
def main() -> None:
    ...
    for subset in hermes_cfg["subsets"]:
        try:
            ...
            # Try as data_files or without config name
            try:
