...
def main() -> None:
    ...
    # Try llm-compressor first; if it can't IMPORT (e.g. broken torchvision
    # + nms operator), surface that loudly. If it imports but the run fails,
    # also surface (don't silently fall through to a broken auto-gptq).
    # v2 lesson: silent-fallback caused empty quantized/ shipped as exit=0.
    try:
        quantize_with_llmcompressor()
    except ImportError as e:
        log(f"  llm-compressor import failed: {e}")
        log("  trying auto-gptq fallback")
        quantize_with_autogptq()
    except Exception as e:
        # llm-compressor was importable but the run threw — propagate; don't
        # silently fall back, because that hides real bugs.
        log(f"  llm-compressor run failed: {type(e).__name__}: {e}")
        raise

    # Verify quantized/ has actual model files (not just a tokenizer).
    # In v2 the quantize call exited 'cleanly' but produced no model
    # safetensors — the orchestrator then reported exit=0 and moved on.
    safetensors_files = list(config.QUANTIZED_DIR.glob("*.safetensors"))
    if not safetensors_files:
        raise RuntimeError(
            f"Quantize produced no .safetensors in {config.QUANTIZED_DIR}. "
            "Refusing to mark step as success (this was the v2 silent-fail)."
        )
