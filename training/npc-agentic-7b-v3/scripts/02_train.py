...
def build_assistant_spans(
    ...
    correspond to each assistant message's body **including the closing
    `<|im_end|>` turn marker**.

    v1/v2 BUG FIX
    -------------
    The earlier version of this function ended each span at
    ``rendered.find(end_marker, body_start)`` — i.e. it stopped JUST
    BEFORE the closing tag. The closing token therefore got -100 in the
    label tensor and the model never received gradient on the EOS. At
    inference, v1/v2 could not terminate cleanly: they would generate
    answers correctly, then lapse into base-Qwen pretraining priors
    (synthetic Human:/Assistant: turns, repeated </details>, hallucinated
    <thoughts>/<chat_history> tags). v3 includes the closer in the span
    by adding ``+ len(end_marker)``. See repo paper for the full
    post-mortem.
    ...
    for m in messages:
        ...
        if marker_pos < 0:
            # Unterminated — extend to end of string
            body_end = len(rendered)
        else:
            # v3 FIX: include the closing <|im_end|> in the span so the
            # EOS token receives gradient during training.
...
def verify_eos_in_loss(
    ...
    """
    Pre-train probe: confirm <|im_end|> tokens are unmasked in the
    label tensors. Returns {eos_total, eos_unmasked, samples}.
    The trainer should assert eos_unmasked > 0 (ideally > eos_total/2,
    where the rest are system+user-turn closers which stay masked).
    """
    total = 0
    unmasked = 0
    for s in samples:
        for tok, lbl in zip(s["input_ids"], s["labels"]):
            if tok == eos_token_id:
                total += 1
                if lbl != -100:
                    unmasked += 1
    return {"eos_total": total, "eos_unmasked": unmasked, "samples": len(samples)}
...
def main() -> None:
    ...
    # ── v3 EOS-in-loss verification probe (catches regression of v1/v2 bug) ──
    # If <|im_end|> tokens are masked from the loss, the model never learns
    # to terminate cleanly at inference. Sample 50 train rows; assert > 0
    # of their <|im_end|> tokens are unmasked. Fail-fast before training.
    eos_check_samples = [train_ds[i] for i in _rand.sample(range(len(train_ds)), min(50, len(train_ds)))]
    eos_stats = verify_eos_in_loss(eos_check_samples, eos_token_id=tokenizer.eos_token_id)
    log(f"  EOS-in-loss probe: {eos_stats['eos_unmasked']} / {eos_stats['eos_total']} "
        f"<|im_end|> tokens unmasked across {eos_stats['samples']} samples")
    if eos_stats["eos_unmasked"] == 0:
        raise RuntimeError(
            "EOS-IN-LOSS BUG REGRESSION: zero <|im_end|> tokens unmasked. "
            "The v1/v2 bug has come back. Check build_assistant_spans() — "
            "the closing marker must be INCLUDED in the body span, not "
            "excluded. Refusing to train; you would ship another model "
            "that cannot terminate at inference time."
        )
    log(f"  ✓ EOS-in-loss check passed.")
